...
func TestReleaseBuildNormalizesTagVersion(t *testing.T) {
	...
	text := string(data)
	for _, expected := range []string{"AIC_BUILD_VERSION:-3.2.0", "command_name=agentsync", "primary_artifact_prefix=agentsync", "legacy_artifact_prefix=aic", "[vV]*) version=${version#?}"} {
