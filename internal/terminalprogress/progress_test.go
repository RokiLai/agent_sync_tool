...
func TestInteractiveDownloadProgress(t *testing.T) {
	...
	r := New(&out, true)
	r.Update(upgrade.Progress{Stage: "download", Name: "aic_Darwin_arm64", Downloaded: 512, Total: 1024})
	r.Update(upgrade.Progress{Stage: "download", Name: "aic_Darwin_arm64", Downloaded: 1024, Total: 1024, Done: true})
	got := out.String()
...
func TestNonInteractiveProgressHasStableLines(t *testing.T) {
	var out bytes.Buffer
	r := New(&out, false)
	r.Update(upgrade.Progress{Stage: "download", Name: "aic_Linux_x86_64", Downloaded: 2048, Total: 2048})
	r.Update(upgrade.Progress{Stage: "download", Name: "aic_Linux_x86_64", Downloaded: 2048, Total: 2048, Done: true})
...
func TestInteractiveFailureClearsDynamicLine(t *testing.T) {
	var out bytes.Buffer
	r := New(&out, true)
	r.Update(upgrade.Progress{Stage: "download", Name: "aic", Downloaded: 10})
	r.Fail()
...
func TestInteractiveStageLabelsAndUnknownLength(t *testing.T) {
	var out bytes.Buffer
	r := New(&out, true)
	r.Update(upgrade.Progress{Stage: "download", Name: "aic", Downloaded: 2048, Total: -1})
