...
func TestUpgradeChecksConfirmsAndRendersProgress(t *testing.T) {
	...
	for _, want := range []string{"当前版本：v3.2.1", "最新版本：v3.3.0", "100%", "升级成功：v3.2.1 → v3.3.0"} {
...
func TestHelpAndVersion(t *testing.T) {
	for _, test := range []struct {
		args     []string
		expected string
	}{{nil, "用法：agentsync"}, {[]string{"--version"}, "ai-instructions 3.2.1\n"}, {[]string{"-V"}, "ai-instructions 3.2.1\n"}} {
